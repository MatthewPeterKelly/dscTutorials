...
%
% This problem is solved using multiple shooting with Euler intergration.
% 

%%% Problem Parameters:
param.maxWaterSpeed = 0.2;   %(m/s)
param.riverWidth = 10.0;  %(m)
param.boatSpeed = 2.0;  %(m/s)

%%% Transcription Parameters:
param.nSegment = 1;  % Trajectory broken into this many segments
param.nSubStep = 20;  % Number of integrations steps per segment
...
[time, state, control, exitFlag, output] = ...
    multipleShooting_Euler(bound, guess, param);
...
subplot(3,1,3);
stairs(time, control,'LineWidth',2);
ylabel('u')
xlabel('t');
