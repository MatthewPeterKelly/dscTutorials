function [dZ, C, E] = dynamics_slidePos(~,Z,P)
...
if isinf(u)
    ...
    dZ(4,:) = (I.*g - L.^3.*dth.^2.*m.*cos(th) + L.^2.*g.*m.*cos(th).^2 - I.*L.*dth.^2.*cos(th))./(L.^2.*m.*cos(th).*sin(th));
...
elseif u==0
    ...
    dZ(4,:) = -(I.*g.*u + L.^3.*dth.^2.*m.*sin(th) + I.*L.*dth.^2.*sin(th) - I.*L.*dth.^2.*u.*cos(th) - L.^3.*dth.^2.*m.*u.*cos(th) + L.^2.*g.*m.*u.*cos(th).^2 - L.^2.*g.*m.*cos(th).*sin(th))./(I + L.^2.*m - L.^2.*m.*cos(th).^2 - L.^2.*m.*u.*cos(th).*sin(th));
