...
SolnOne = FileWritingSetup.SolnOne;
SolnTwo = FileWritingSetup.SolnTwo;
...
%~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~%
    ...
    for idxState=1:size(States,1)
        fprintf(fid,[States{idxState,1} ' = StateBefore(' num2str(idxState) '); %% ' ...
            States{idxState,2} '\n']);
    end %idxState
    fprintf(fid,'\n');
    ...
    N = num2str(Ndof);
    fprintf(fid,'StateAfter = zeros(size(StateBefore));\n');
    fprintf(fid,['StateAfter(1:' N ',:) = '...
        'StateBefore(1:' N ',:); %% Configuration not changed by collision\n']);
    for idx=1:Ndof
        writeIndex = idx + Ndof;
        writeName = [States{writeIndex,1} '_plus'];
        vectorizedExpression = char(SolnOne.(writeName));
        fprintf(fid,['StateAfter(' num2str(writeIndex) ') = ' ...
...
%~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~%
    ...
    for idx=1:Ndof
        writeIndex = idx + Ndof;
        writeName = [States{writeIndex,1} '_plus'];
        vectorizedExpression = char(SolnTwo.(writeName));
        fprintf(fid,['StateAfter(' num2str(writeIndex) ') = ' ...
            vectorizedExpression ';\n']);
    end %idxAcc
    fprintf(fid,'\n');
