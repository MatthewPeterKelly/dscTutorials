...
CommonExpressions = FileWritingSetup.CommonExpressions;
CE = CommonExpressions;
...
for idxPhase = 1:size(FileData,1)
    ...
    for i=1:length(fNames)
        if strcmp(fNames{i},'L1') || strcmp(fNames{i},'L2')  %Need these ones!
            fprintf(fid,[fNames{i} ' = ' vectorize(Kinematics.(fNames{i}),CE) ';\n']);  
        else  %Don't need them - comment out
            fprintf(fid,['%% ' fNames{i} ' = ' char(Kinematics.(fNames{i})) ';\n']);  
        end
    end
    for i=1:size(CommonExpressions,1)
        fprintf(fid,[CommonExpressions{i,1} ' = ' CommonExpressions{i,2} ';\n']);   
    end
    fprintf(fid,'\n');
    ...
    for idxAcc=1:Ndof
        ...
        writeName = ['d' States{writeIndex,1}];
        writeData = Dyn.(FileData{idxPhase,1}).(writeName);
        vectorizedExpression = vectorize(writeData,CE);
    ...
    fprintf(fid,['contactForces = zeros(size(States,2),' ...
        num2str(size(Contacts,1)) ');\n']);
    for idxContact=1:size(Contacts,1)
...
function characterExpression = vectorize(symbolicExpression,CommonExpressions)

if symbolicExpression == sym('0');
    characterExpression = 'zeros(size(States,1),1)';
else
    ...
    for i=1:size(CommonExpressions,1)
       characterExpression = strrep(characterExpression,...
           CommonExpressions{i,2},CommonExpressions{i,1}); 
    end
