...
for idxAct=1:size(Actuators,1)    
    ...
    if strcmp(Actuators{idxAct,1},'T1')
        ...
        fprintf(fid,['    ' Actuators{idxAct,1} ' = zeros(size(Actuators,1),1); %%Foot One not in contact with ground!\n']);
        fprintf(fid,'else\n');
    ...
    elseif strcmp(Actuators{idxAct,1},'T2')
        ...
        fprintf(fid,['    ' Actuators{idxAct,1} ' = zeros(size(Actuators,1),1); %%Foot Two not in contact with ground!\n']);
        fprintf(fid,'else\n');
        fprintf(fid,['    ' Actuators{idxAct,1} ' = Actuators(:,' num2str(idxAct) '); %% ' Actuators{idxAct,2} '\n']);
        fprintf(fid,'end\n');      
    else
...
for i=1:length(fNames)
    if strcmp(fNames{i},'th1') || strcmp(fNames{i},'th2')  %Don't need these!
