...
ddx2 = 0; %(m) Foot Two horizontal position
ddy2 = 0; %(m) Foot Two vertical position
...
dStates(:,9) = (T1.*y0 - T1.*y1 + Thip.*y0 - Thip.*y1 + H1.*L1s - F1.*L1.*x0 + F1.*L1.*x1)./(L1s.*m1);
dStates(:,10) = -(T1.*x0 - T1.*x1 + Thip.*x0 - Thip.*x1 - L1s.*V1 + F1.*L1.*y0 - F1.*L1.*y1 + L1s.*g.*m1)./(L1s.*m1);
dStates(:,11) = zeros(size(States,1),1);
dStates(:,12) = zeros(size(States,1),1);
...
contactForces(:,3) = (T2.*y2 - T2.*y0 + Thip.*y0 - Thip.*y2 + F2.*L2.*x0 - F2.*L2.*x2 + L2s.*ddx2.*m2)./L2s;
contactForces(:,4) = (T2.*x0 - T2.*x2 - Thip.*x0 + Thip.*x2 + F2.*L2.*y0 - F2.*L2.*y2 + L2s.*ddy2.*m2 + L2s.*g.*m2)./L2s;
